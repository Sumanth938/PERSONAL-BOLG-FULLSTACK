...
def authenticate_user(username: str, password: str, db):
    ...
    user = db.query(User)\
        .filter(User.is_active == True,User.email == username)\
        .first()
    if not user:
        return False
...
def create_access_token(user_id: int,user_email:str,
    ...
    encode = {"user_id": user_id,"email":user_email}
...
def get_current_user(token: str = Depends(oauth2_bearer)):
    ...
    try:
        ...
        return {'user_id': user_details.id,'email':user_details.email}
...
def create_new_user(
    ...
    try:
        ...
        if len(fullname) <6 :
            return JSONResponse({"detail":"FULLNAME MUST BE GREATER THAN 5 CHARECTERS"},status_code=400)
...
def login_for_access_token(form_data : OAuth2PasswordRequestForm = Depends(),
    ...
    try:
        ...
        token = create_access_token(user.id, user.email,expires_delta=token_expires)
